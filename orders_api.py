...
async def demo_run_state(token: str, command_id: str,
    ...
    if r.get("status") == "succeeded" and r.get("ticket"):
      ...
      # never hide a broker result: the trade already happened, the row
      # already says so, and the caller is entitled to read it. This
      # endpoint once returned 500 for twenty minutes because a delivery
      # bug propagated out of here while the order sat filled.
      try:
